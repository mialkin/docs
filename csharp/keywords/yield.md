...
When you use `yiled return` the compiler will automatically [generate a class](../lowering.md#yield-return) that implements both `IEnumerable<T>` and `IEnumerator<T>` (in the same class).
